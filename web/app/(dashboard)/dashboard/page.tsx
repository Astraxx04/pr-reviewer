  ...
  return (
    <div className="space-y-8">
      ...
      <Card>
        ...
        <CardContent>
          {recent.length === 0 ? (
            <p className="text-muted-foreground text-base">No reviews yet.</p>
          ) : (
            <ul className="divide-y" role="list" aria-label="Recent reviews">
              {recent.map((r) => (
                <li key={r.ID} className="flex items-center justify-between py-4">
                  <div>
                    <p className="text-base font-medium">Review #{r.ID}</p>
                    <p className="text-sm text-muted-foreground">
                      <time dateTime={r.CreatedAt}>{new Date(r.CreatedAt).toLocaleString()}</time>
                    </p>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="text-base font-mono" aria-label={`Score: ${r.Score} out of 100`}>
                      {r.Score}/100
                    </span>
                    <Badge className="text-sm px-3 py-1" variant={statusVariant(r.Status)}>{r.Status}</Badge>
                  </div>
                </li>
              ))}
            </ul>
...
function StatCard({ title, value }: { title: string; value: string | number }) {
  return (
    <Card>
      ...
      <CardContent>
        <p className="text-4xl font-bold" aria-label={`${title}: ${value}`}>{value}</p>
      </CardContent>
    </Card>
  );
}
...
function HealthMetric({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className="space-y-1">
      ...
      <p
        className={`text-xl font-semibold ${highlight ? "text-destructive" : ""}`}
